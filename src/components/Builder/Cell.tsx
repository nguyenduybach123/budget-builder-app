...
export const Cell = ({id, parentId, value}: CellType & {parentId: number}) => {
    ...
    return (
        <td className={cn("h-10 w-32 max-w-32 border-2 hover:border-indigo-500/100 hover:bg-slate-300 group/title cursor-cell", (id === currentPos?.cellId && parentId === currentPos.rowId) ? "border-indigo-500/100" : "border-indigo-500/50")}
